...
- 活动配置中心通过唯一活动 Key 与活动类型管理不同活动；当前已接入“充值活动”类型。
- 充值活动类型配置 $1,000 与 $1,700 两个累计充值阶梯，支持动态增删阶梯，并可从全部宠物蛋配置中添加多个奖励及数量。
...
- 活动配置：按关键词与活动类型筛选、新建与编辑、活动 Key 校验；充值活动支持阶梯增删、门槛递增校验、添加与移除宠物蛋奖励。
- 宠物与宠物蛋列表右侧仅保留编辑操作。
- 配置保存反馈。
...
- 2026-08-11 活动配置补充：升级为通用活动配置中心，通过活动类型加载对应规则；当前充值活动支持动态阶梯及宠物蛋奖励，固定的钱包跳转、累计周期与重置时间不展示。
