...
- 宠物蛋与宠物资源均以真实文件配置对象呈现，WebP / MP4 格式说明清晰。
- 未使用营销插画、装饰图片或无依据资源。
...
- 宠物覆盖：WebP 封面、默认名称、八个用户区域名称、A/S/SS/SSS 品级。
- 宠物列表将宠物 ID 作为独立列展示，便于运营检索和研发对照。
- 三星形态覆盖：一星初始、二星等级 30、三星等级 50；每个形态均含封面缩略图、待机资源和互动资源。
- 每个星级的封面缩略图均为必填 WebP 静态资源，文件名支持在上传区域内截断展示。
...
- 2026-08-10 形态资源补充：一星、二星、三星形态分别新增必填的 WebP 封面缩略图上传项。
