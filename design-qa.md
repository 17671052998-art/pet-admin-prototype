...
- 宠物蛋与宠物资源均以真实文件配置对象呈现；宠物形态的待机、互动资源统一支持 WebP / MP4。
- 未使用营销插画、装饰图片或无依据资源。
...
- 已上传的 WebP / MP4 形态资源支持悬停预览动画，并保留单项“移除上传”操作；新选择的本地文件可直接显示真实预览。
...
- 2026-08-11 形态资源合并：取消客户端与 H5 分组，待机、互动资源统一支持 WebP / MP4，并补充悬停预览与移除上传。
- 2026-08-18 形态外部展示补充：一星、二星、三星形态分别新增“个人主页 / 资料卡片展示资源”，均支持 PNG / WebP 上传、悬停预览与移除；客户端按当前形态读取对应资源。
- 2026-08-11 活动配置补充：升级为通用活动配置中心，通过活动类型加载对应规则；当前充值活动支持动态阶梯及宠物蛋奖励，固定的钱包跳转、累计周期与重置时间不展示。
- 2026-08-11 活动表单对齐修订：活动 Key、活动名称与活动类型统一标签行、38px 控件行和说明文案占位，消除左右输入框高度与垂直起点偏差。
